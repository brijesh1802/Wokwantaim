...
const ManageEntitiesSection = () => {
  ...
  const fetchEntities = useCallback(async () => {
    try {
      ...
      const entityMap = {
        company: {
          apiEndpoint: "/api/v1/admin/getCompanies",
          setter: setCompanies,
        },
        industry: {
          apiEndpoint: "/api/v1/admin/getIndustries",
          setter: setIndustries,
        },
        jobType: {
          apiEndpoint: "/api/v1/admin/getJobTypes",
          setter: setJobTypes,
        },
        experienceLevel: {
          apiEndpoint: "/api/v1/admin/getExperienceLevels",
          setter: setExperienceLevels,
        },
      };
      const { apiEndpoint, setter } = entityMap[activeEntity];
      const url = `${import.meta.env.VITE_BASE_URL}${apiEndpoint}`;
      ...
      if (!response.ok)
        throw new Error(
          `Failed to fetch ${activeEntity}: ${response.status} ${response.statusText}`
        );
      const data = await response.json();
      setter(data);
      if (data.length === 0)
        setError(`No ${entityNameMap[activeEntity]} found.`);
    } catch (err) {
      console.error(`Error fetching ${activeEntity}:`, err);
      setError(err.message);
    } finally {
  ...
  const handleAddEntity = async () => {
    ...
    try {
      ...
      const entityMap = {
        company: {
          apiEndpoint: "/api/v1/admin/addCompany",
          setter: setCompanies,
        },
        industry: {
          apiEndpoint: "/api/v1/admin/addIndustry",
          setter: setIndustries,
        },
        jobType: {
          apiEndpoint: "/api/v1/admin/addJobType",
          setter: setJobTypes,
        },
        experienceLevel: {
          apiEndpoint: "/api/v1/admin/addExperienceLevel",
          setter: setExperienceLevels,
        },
      };
      const { apiEndpoint } = entityMap[activeEntity];
      ...
      if (activeEntity === "company" && newEntityLogo) {
        ...
        body = new FormData();
        body.append("name", newEntityName);
        body.append("logo", newEntityLogo);
      ...
      if (!response.ok)
        throw new Error(
          `Failed to add ${activeEntity}: ${response.status} ${response.statusText}`
        );
  ...
  const handleDeleteEntity = async () => {
    ...
    try {
      ...
      const entityMap = {
        company: {
          apiEndpoint: "/api/v1/admin/deleteCompany",
          setter: setCompanies,
        },
        industry: {
          apiEndpoint: "/api/v1/admin/deleteIndustry",
          setter: setIndustries,
        },
        jobType: {
          apiEndpoint: "/api/v1/admin/deleteJobType",
          setter: setJobTypes,
        },
        experienceLevel: {
          apiEndpoint: "/api/v1/admin/deleteExperienceLevel",
          setter: setExperienceLevels,
        },
      };
      const { apiEndpoint } = entityMap[activeEntity];
      ...
      if (!response.ok)
        throw new Error(
          `Failed to delete ${activeEntity}: ${response.status} ${response.statusText}`
        );
      closeDeleteConfirmation();
      fetchEntities();
    } catch (err) {
  ...
  const getIcon = (entity) => {
    switch (entity) {
      case "company":
        return <FiBriefcase className="mr-2" size={20} />;
      case "industry":
        return <FiTool className="mr-2" size={20} />;
      case "jobType":
        return <FiFile className="mr-2" size={20} />;
      case "experienceLevel":
        return <FiUser className="mr-2" size={20} />;
      default:
        return <FiUser className="mr-2" size={20} />;
    }
  };
  ...
  const filteredEntities = getEntities().filter(
    (entity) =>
      entity.name &&
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-gray-800">
                  Add {entityNameMap[activeEntity].slice(0, -1)}
                </h3>
                <button
                  onClick={() => {
                    setShowAddModal(false);
              ...
              <input
                type="text"
                placeholder={`Enter ${entityNameMap[activeEntity].slice(
                  0,
                  -1
                )} name`}
                value={newEntityName}
