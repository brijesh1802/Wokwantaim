...
const JobList = () => {
  ...
  const handleRefresh = (resetTitle = true) => {
    console.log("Before Reset:", checkedOptions, selectedRadio);
    ...
    setFilteredJobRole(
      ...
      (title.trim() !== "" ? filteredSearchJobRole : jobs).filter((job) => {
        ...
        const matchTitleAndCompany =
          ...
          // const matchTitle=title.toLowerCase().includes(job.title.toLowerCase())
  ...
  }, [currentJobRole, jobs, filteredSearchJobRole,title]);

  console.log("curent JobRole",currentJobRole);
      ...
      <div className="gap-4">
        ...
        <div className="px-4 py-8 -mt-10 bg-gray-100 rounded-md lg:mx-8 lg:-mt-16 md:shadow-xl">
          ...
          <div className="flex justify-center mt-4 md:mx-40 gap-3">
            ...
            <button className="hover:text-orange-500" onClick={()=>handleRefresh(true)}>
              <RefreshCcw />{" "}
            </button>
          </div>
        </div>
