...
const PersonalProjects = ({ personalProjects, setPersonalProjects }) => {
  ...
  return (
    <div className="mt-8 mb-3 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {personalProjects.length > 0 ? (
        personalProjects.map((project, index) => (
          <div
            key={index}
            className="relative flex flex-col p-6 bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300"
          >
            <button
              onClick={() => handleDelete(index)}
